...
window.loadMyLeaves = async function() {
    ...
    try {
        ...
        container.innerHTML = leaves.map(l => {
            ...
            if(l.status === 'Approved') { color = '#16a34a'; bgLight = '#bbf7d0'; text = 'Approved'; }
            if(l.status === 'Rejected') { color = '#dc2626'; bgLight = '#fecaca'; text = 'Rejected'; }
    ...
    } catch (e) { console.error("Error loading my leaves:", e); }
};
...
window.loadPendingLeaves = async function() {
    ...
    try {
        ...
        `).join('');
    } catch (e) { console.error("Error loading pending leaves:", e); }
};
...
window.processLeave = async function(leaveId, userId, newStatus) {
    ...
    try {
        await window.sb.from('leaves').update({ status: newStatus }).eq('id', leaveId);
